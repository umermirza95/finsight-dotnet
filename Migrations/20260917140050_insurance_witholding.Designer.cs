// <auto-generated />
using System;
using System.Collections.Generic;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;

#nullable disable

namespace finsight_dotnet.Migrations
{
    [DbContext(typeof(AppDbContext))]
    [Migration("20260917140050_insurance_witholding")]
    partial class insurance_witholding
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "9.0.9")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("Finsight.Models.FSBudget", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("FSCurrencyCode")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("FSUserId")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Frequency")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateOnly>("StartDate")
                        .HasColumnType("date");

                    b.HasKey("Id");

                    b.HasIndex("FSCurrencyCode");

                    b.HasIndex("FSUserId");

                    b.ToTable("FSBudgets");
                });

            modelBuilder.Entity("Finsight.Models.FSBudgetCategory", b =>
                {
                    b.Property<Guid>("BudgetId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("CategoryId")
                        .HasColumnType("uuid");

                    b.HasKey("BudgetId", "CategoryId");

                    b.HasIndex("CategoryId");

                    b.ToTable("FSBudgetCategories");
                });

            modelBuilder.Entity("Finsight.Models.FSBudgetPeriod", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<decimal>("Amount")
                        .HasColumnType("decimal(18,2)");

                    b.Property<Guid>("BudgetId")
                        .HasColumnType("uuid");

                    b.Property<DateOnly>("StartDate")
                        .HasColumnType("date");

                    b.HasKey("Id");

                    b.HasIndex("BudgetId");

                    b.ToTable("FSBudgetPeriods");
                });

            modelBuilder.Entity("Finsight.Models.FSCategory", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("FSUserId")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("FSUserId");

                    b.ToTable("Categories");
                });

            modelBuilder.Entity("Finsight.Models.FSClosedTrade", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<decimal>("ClosedQuantity")
                        .HasColumnType("decimal(18,2)");

                    b.Property<string>("FSUserId")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<decimal>("NetProfit")
                        .HasColumnType("decimal(18,2)");

                    b.Property<string>("OrderCloseId")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("OrderOpenId")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("FSUserId");

                    b.HasIndex("OrderCloseId");

                    b.HasIndex("OrderOpenId");

                    b.ToTable("FSClosedTrades");
                });

            modelBuilder.Entity("Finsight.Models.FSCurrency", b =>
                {
                    b.Property<string>("Code")
                        .HasColumnType("text");

                    b.HasKey("Code");

                    b.ToTable("FSCurrencies", (string)null);
                });

            modelBuilder.Entity("Finsight.Models.FSExchangeRate", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<DateOnly>("Date")
                        .HasColumnType("date");

                    b.Property<decimal>("ExchangeRate")
                        .HasColumnType("numeric");

                    b.Property<string>("From")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("To")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("To");

                    b.HasIndex("From", "To", "Date")
                        .IsUnique();

                    b.ToTable("FSExchangeRates");
                });

            modelBuilder.Entity("Finsight.Models.FSFile", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<Guid?>("FSTransactionId")
                        .HasColumnType("uuid");

                    b.Property<string>("FSUserId")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("FileName")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("FilePath")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateTime>("UploadedAt")
                        .HasColumnType("timestamp with time zone");

                    b.HasKey("Id");

                    b.HasIndex("FSTransactionId");

                    b.HasIndex("FSUserId");

                    b.ToTable("FSFiles");
                });

            modelBuilder.Entity("Finsight.Models.FSImportedTransaction", b =>
                {
                    b.Property<string>("Id")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<decimal>("Amount")
                        .HasColumnType("decimal(18,2)");

                    b.Property<string>("BankName")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateOnly>("Date")
                        .HasColumnType("date");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("FSCurrencyCode")
                        .IsRequired()
                        .HasMaxLength(3)
                        .HasColumnType("character varying(3)");

                    b.Property<Guid?>("FSTransactionId")
                        .HasColumnType("uuid");

                    b.Property<string>("FSUserId")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<Guid?>("FSWalletId")
                        .HasColumnType("uuid");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("boolean");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("FSTransactionId");

                    b.HasIndex("FSUserId");

                    b.HasIndex("FSWalletId");

                    b.ToTable("FSImportedTransactions");
                });

            modelBuilder.Entity("Finsight.Models.FSInjectedCapital", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<decimal>("Amount")
                        .HasColumnType("decimal(18,2)");

                    b.Property<DateTime>("Date")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("FSUserId")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("FSUserId");

                    b.ToTable("FSInjectedCapitals");
                });

            modelBuilder.Entity("Finsight.Models.FSInsurancePayout", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<decimal>("CoveredAmount")
                        .HasColumnType("decimal(18,2)");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("FSClosedTradeId")
                        .HasColumnType("uuid");

                    b.Property<string>("FSUserId")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("FSClosedTradeId")
                        .IsUnique();

                    b.HasIndex("FSUserId");

                    b.ToTable("FSInsurancePayouts");
                });

            modelBuilder.Entity("Finsight.Models.FSNotification", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("FSUserId")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<bool>("IsRead")
                        .HasColumnType("boolean");

                    b.Property<string>("Message")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("FSUserId");

                    b.ToTable("FSNotifications");
                });

            modelBuilder.Entity("Finsight.Models.FSProfitDistribution", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<decimal>("Amount")
                        .HasColumnType("decimal(18,2)");

                    b.Property<DateTime>("Date")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("DistributionType")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("FSUserId")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("FSUserId");

                    b.ToTable("FSProfitDistributions");
                });

            modelBuilder.Entity("Finsight.Models.FSPushSubscription", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("Auth")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("Endpoint")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("FSUserId")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("P256dh")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("FSUserId");

                    b.ToTable("FSPushSubscriptions");
                });

            modelBuilder.Entity("Finsight.Models.FSSubCategory", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<Guid>("FSCategoryId")
                        .HasColumnType("uuid");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("FSCategoryId");

                    b.ToTable("SubCategories");
                });

            modelBuilder.Entity("Finsight.Models.FSTrade", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<decimal>("Commission")
                        .HasColumnType("decimal(18,2)");

                    b.Property<DateTime>("Date")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("ExternalId")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("FSUserId")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<decimal>("Quantity")
                        .HasColumnType("decimal(18,2)");

                    b.Property<decimal>("SharesLeft")
                        .HasColumnType("decimal(18,2)");

                    b.Property<string>("Ticker")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("TradeDirection")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<decimal>("TradePrice")
                        .HasColumnType("decimal(18,2)");

                    b.HasKey("Id");

                    b.HasIndex("ExternalId")
                        .IsUnique();

                    b.HasIndex("FSUserId");

                    b.ToTable("FSTrades");
                });

            modelBuilder.Entity("Finsight.Models.FSTradingConfig", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("AlpacaApiKey")
                        .HasColumnType("text");

                    b.Property<string>("AlpacaApiSecret")
                        .HasColumnType("text");

                    b.Property<bool>("AutoTrade")
                        .HasColumnType("boolean");

                    b.Property<decimal>("DistancePerTranche")
                        .HasColumnType("decimal(18,2)");

                    b.Property<string>("FSUserId")
                        .HasColumnType("text");

                    b.Property<decimal?>("InsuranceWithold")
                        .HasColumnType("decimal(5,2)");

                    b.Property<bool>("LogsOnly")
                        .HasColumnType("boolean");

                    b.Property<string>("ServerIp")
                        .HasColumnType("text");

                    b.Property<int?>("ServerPort")
                        .HasColumnType("integer");

                    b.Property<decimal>("SharesPerTranche")
                        .HasColumnType("decimal(18,2)");

                    b.Property<string>("Ticker")
                        .HasColumnType("text");

                    b.Property<decimal>("TradingCapital")
                        .HasColumnType("decimal(18,2)");

                    b.Property<decimal>("TrancheSize")
                        .HasColumnType("decimal(18,2)");

                    b.HasKey("Id");

                    b.HasIndex("FSUserId");

                    b.HasIndex("ServerIp")
                        .IsUnique();

                    b.ToTable("TradingConfigs");
                });

            modelBuilder.Entity("Finsight.Models.FSTransaction", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<decimal>("Amount")
                        .HasColumnType("decimal(18,2)");

                    b.Property<string>("Comment")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<DateOnly>("Date")
                        .HasColumnType("date");

                    b.Property<Guid>("FSCategoryId")
                        .HasColumnType("uuid");

                    b.Property<string>("FSCurrencyCode")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<Guid?>("FSSubCategoryId")
                        .HasColumnType("uuid");

                    b.Property<string>("FSUserId")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<Guid?>("FSWalletId")
                        .HasColumnType("uuid");

                    b.Property<string>("Mode")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("SubType")
                        .HasColumnType("text");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.HasKey("Id");

                    b.HasIndex("FSCategoryId");

                    b.HasIndex("FSCurrencyCode");

                    b.HasIndex("FSSubCategoryId");

                    b.HasIndex("FSUserId");

                    b.HasIndex("FSWalletId");

                    b.ToTable("Transactions");
                });

            modelBuilder.Entity("Finsight.Models.FSTransactionEmail", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.PrimitiveCollection<List<string>>("From")
                        .IsRequired()
                        .HasColumnType("text[]");

                    b.Property<string>("Html")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateTime>("ReceivedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("Subject")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Text")
                        .IsRequired()
                        .HasColumnType("text");

                    b.PrimitiveCollection<List<string>>("To")
                        .IsRequired()
                        .HasColumnType("text[]");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.ToTable("FSTransactionEmails");
                });

            modelBuilder.Entity("Finsight.Models.FSTransactionSuggestion", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<decimal>("Amount")
                        .HasColumnType("decimal(18,2)");

                    b.Property<string>("Comment")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<DateOnly>("Date")
                        .HasColumnType("date");

                    b.Property<Guid>("FSCategoryId")
                        .HasColumnType("uuid");

                    b.Property<string>("FSCurrencyCode")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<Guid?>("FSSubCategoryId")
                        .HasColumnType("uuid");

                    b.Property<string>("FSUserId")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Mode")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("SubType")
                        .HasColumnType("text");

                    b.Property<string>("TransactionExternalId")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.HasKey("Id");

                    b.HasIndex("FSCategoryId");

                    b.HasIndex("FSCurrencyCode");

                    b.HasIndex("FSSubCategoryId");

                    b.HasIndex("FSUserId");

                    b.ToTable("FSTransactionSuggestions");
                });

            modelBuilder.Entity("Finsight.Models.FSUser", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("text");

                    b.Property<int>("AccessFailedCount")
                        .HasColumnType("integer");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .HasColumnType("text");

                    b.Property<string>("DefaultCurrency")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("text")
                        .HasDefaultValue("USD");

                    b.Property<string>("Email")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<bool>("EmailConfirmed")
                        .HasColumnType("boolean");

                    b.Property<string>("IBKRQueryId")
                        .HasColumnType("text");

                    b.Property<string>("IBKRToken")
                        .HasColumnType("text");

                    b.Property<bool>("LockoutEnabled")
                        .HasColumnType("boolean");

                    b.Property<DateTimeOffset?>("LockoutEnd")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("NormalizedEmail")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("NormalizedUserName")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("PasswordHash")
                        .HasColumnType("text");

                    b.Property<string>("PhoneNumber")
                        .HasColumnType("text");

                    b.Property<bool>("PhoneNumberConfirmed")
                        .HasColumnType("boolean");

                    b.Property<string>("PreferredBroker")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("SecurityStamp")
                        .HasColumnType("text");

                    b.Property<bool>("TwoFactorEnabled")
                        .HasColumnType("boolean");

                    b.Property<string>("UserName")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.HasKey("Id");

                    b.HasIndex("DefaultCurrency");

                    b.HasIndex("Email")
                        .IsUnique();

                    b.HasIndex("NormalizedEmail")
                        .HasDatabaseName("EmailIndex");

                    b.HasIndex("NormalizedUserName")
                        .IsUnique()
                        .HasDatabaseName("UserNameIndex");

                    b.ToTable("AspNetUsers", (string)null);
                });

            modelBuilder.Entity("Finsight.Models.FSWallet", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreationDate")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("FSCurrencyCode")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("FSUserId")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<decimal>("InitialBalance")
                        .HasColumnType("decimal(18,2)");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<int>("Order")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("FSCurrencyCode");

                    b.HasIndex("FSUserId");

                    b.ToTable("FSWallets");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRole", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("text");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .HasColumnType("text");

                    b.Property<string>("Name")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("NormalizedName")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.HasKey("Id");

                    b.HasIndex("NormalizedName")
                        .IsUnique()
                        .HasDatabaseName("RoleNameIndex");

                    b.ToTable("AspNetRoles", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<string>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("ClaimType")
                        .HasColumnType("text");

                    b.Property<string>("ClaimValue")
                        .HasColumnType("text");

                    b.Property<string>("RoleId")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("RoleId");

                    b.ToTable("AspNetRoleClaims", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<string>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("ClaimType")
                        .HasColumnType("text");

                    b.Property<string>("ClaimValue")
                        .HasColumnType("text");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserClaims", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<string>", b =>
                {
                    b.Property<string>("LoginProvider")
                        .HasColumnType("text");

                    b.Property<string>("ProviderKey")
                        .HasColumnType("text");

                    b.Property<string>("ProviderDisplayName")
                        .HasColumnType("text");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("LoginProvider", "ProviderKey");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserLogins", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserRole<string>", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("text");

                    b.Property<string>("RoleId")
                        .HasColumnType("text");

                    b.HasKey("UserId", "RoleId");

                    b.HasIndex("RoleId");

                    b.ToTable("AspNetUserRoles", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<string>", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("text");

                    b.Property<string>("LoginProvider")
                        .HasColumnType("text");

                    b.Property<string>("Name")
                        .HasColumnType("text");

                    b.Property<string>("Value")
                        .HasColumnType("text");

                    b.HasKey("UserId", "LoginProvider", "Name");

                    b.ToTable("AspNetUserTokens", (string)null);
                });

            modelBuilder.Entity("Finsight.Models.FSBudget", b =>
                {
                    b.HasOne("Finsight.Models.FSCurrency", null)
                        .WithMany()
                        .HasForeignKey("FSCurrencyCode")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Finsight.Models.FSUser", null)
                        .WithMany()
                        .HasForeignKey("FSUserId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();
                });

            modelBuilder.Entity("Finsight.Models.FSBudgetCategory", b =>
                {
                    b.HasOne("Finsight.Models.FSBudget", "Budget")
                        .WithMany("BudgetCategories")
                        .HasForeignKey("BudgetId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Finsight.Models.FSCategory", "Category")
                        .WithMany("BudgetCategories")
                        .HasForeignKey("CategoryId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Budget");

                    b.Navigation("Category");
                });

            modelBuilder.Entity("Finsight.Models.FSBudgetPeriod", b =>
                {
                    b.HasOne("Finsight.Models.FSBudget", "Budget")
                        .WithMany("Periods")
                        .HasForeignKey("BudgetId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Budget");
                });

            modelBuilder.Entity("Finsight.Models.FSCategory", b =>
                {
                    b.HasOne("Finsight.Models.FSUser", null)
                        .WithMany()
                        .HasForeignKey("FSUserId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();
                });

            modelBuilder.Entity("Finsight.Models.FSClosedTrade", b =>
                {
                    b.HasOne("Finsight.Models.FSUser", "User")
                        .WithMany()
                        .HasForeignKey("FSUserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Finsight.Models.FSTrade", "CloseTrade")
                        .WithMany()
                        .HasForeignKey("OrderCloseId")
                        .HasPrincipalKey("ExternalId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Finsight.Models.FSTrade", "OpenTrade")
                        .WithMany()
                        .HasForeignKey("OrderOpenId")
                        .HasPrincipalKey("ExternalId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("CloseTrade");

                    b.Navigation("OpenTrade");

                    b.Navigation("User");
                });

            modelBuilder.Entity("Finsight.Models.FSExchangeRate", b =>
                {
                    b.HasOne("Finsight.Models.FSCurrency", null)
                        .WithMany()
                        .HasForeignKey("From")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Finsight.Models.FSCurrency", null)
                        .WithMany()
                        .HasForeignKey("To")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();
                });

            modelBuilder.Entity("Finsight.Models.FSFile", b =>
                {
                    b.HasOne("Finsight.Models.FSTransaction", "Transaction")
                        .WithMany("Files")
                        .HasForeignKey("FSTransactionId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("Finsight.Models.FSUser", null)
                        .WithMany()
                        .HasForeignKey("FSUserId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Transaction");
                });

            modelBuilder.Entity("Finsight.Models.FSImportedTransaction", b =>
                {
                    b.HasOne("Finsight.Models.FSTransaction", "FSTransaction")
                        .WithMany("FSImportedTransactions")
                        .HasForeignKey("FSTransactionId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("Finsight.Models.FSUser", null)
                        .WithMany()
                        .HasForeignKey("FSUserId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Finsight.Models.FSWallet", null)
                        .WithMany()
                        .HasForeignKey("FSWalletId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("FSTransaction");
                });

            modelBuilder.Entity("Finsight.Models.FSInjectedCapital", b =>
                {
                    b.HasOne("Finsight.Models.FSUser", "User")
                        .WithMany()
                        .HasForeignKey("FSUserId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("Finsight.Models.FSInsurancePayout", b =>
                {
                    b.HasOne("Finsight.Models.FSClosedTrade", "ClosedTrade")
                        .WithOne("InsurancePayout")
                        .HasForeignKey("Finsight.Models.FSInsurancePayout", "FSClosedTradeId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Finsight.Models.FSUser", "User")
                        .WithMany()
                        .HasForeignKey("FSUserId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("ClosedTrade");

                    b.Navigation("User");
                });

            modelBuilder.Entity("Finsight.Models.FSNotification", b =>
                {
                    b.HasOne("Finsight.Models.FSUser", null)
                        .WithMany()
                        .HasForeignKey("FSUserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Finsight.Models.FSProfitDistribution", b =>
                {
                    b.HasOne("Finsight.Models.FSUser", "User")
                        .WithMany()
                        .HasForeignKey("FSUserId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("Finsight.Models.FSPushSubscription", b =>
                {
                    b.HasOne("Finsight.Models.FSUser", "User")
                        .WithMany()
                        .HasForeignKey("FSUserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("Finsight.Models.FSSubCategory", b =>
                {
                    b.HasOne("Finsight.Models.FSCategory", null)
                        .WithMany("SubCategories")
                        .HasForeignKey("FSCategoryId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Finsight.Models.FSTrade", b =>
                {
                    b.HasOne("Finsight.Models.FSUser", null)
                        .WithMany()
                        .HasForeignKey("FSUserId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();
                });

            modelBuilder.Entity("Finsight.Models.FSTradingConfig", b =>
                {
                    b.HasOne("Finsight.Models.FSUser", null)
                        .WithMany()
                        .HasForeignKey("FSUserId")
                        .OnDelete(DeleteBehavior.Restrict);
                });

            modelBuilder.Entity("Finsight.Models.FSTransaction", b =>
                {
                    b.HasOne("Finsight.Models.FSCategory", null)
                        .WithMany()
                        .HasForeignKey("FSCategoryId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Finsight.Models.FSCurrency", null)
                        .WithMany()
                        .HasForeignKey("FSCurrencyCode")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Finsight.Models.FSSubCategory", null)
                        .WithMany()
                        .HasForeignKey("FSSubCategoryId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("Finsight.Models.FSUser", null)
                        .WithMany()
                        .HasForeignKey("FSUserId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Finsight.Models.FSWallet", null)
                        .WithMany()
                        .HasForeignKey("FSWalletId")
                        .OnDelete(DeleteBehavior.Restrict);
                });

            modelBuilder.Entity("Finsight.Models.FSTransactionEmail", b =>
                {
                    b.HasOne("Finsight.Models.FSUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();
                });

            modelBuilder.Entity("Finsight.Models.FSTransactionSuggestion", b =>
                {
                    b.HasOne("Finsight.Models.FSCategory", null)
                        .WithMany()
                        .HasForeignKey("FSCategoryId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Finsight.Models.FSCurrency", null)
                        .WithMany()
                        .HasForeignKey("FSCurrencyCode")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Finsight.Models.FSSubCategory", null)
                        .WithMany()
                        .HasForeignKey("FSSubCategoryId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("Finsight.Models.FSUser", null)
                        .WithMany()
                        .HasForeignKey("FSUserId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();
                });

            modelBuilder.Entity("Finsight.Models.FSUser", b =>
                {
                    b.HasOne("Finsight.Models.FSCurrency", null)
                        .WithMany()
                        .HasForeignKey("DefaultCurrency")
                        .OnDelete(DeleteBehavior.Restrict);
                });

            modelBuilder.Entity("Finsight.Models.FSWallet", b =>
                {
                    b.HasOne("Finsight.Models.FSCurrency", null)
                        .WithMany()
                        .HasForeignKey("FSCurrencyCode")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Finsight.Models.FSUser", null)
                        .WithMany()
                        .HasForeignKey("FSUserId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityRole", null)
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<string>", b =>
                {
                    b.HasOne("Finsight.Models.FSUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<string>", b =>
                {
                    b.HasOne("Finsight.Models.FSUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserRole<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityRole", null)
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Finsight.Models.FSUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<string>", b =>
                {
                    b.HasOne("Finsight.Models.FSUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Finsight.Models.FSBudget", b =>
                {
                    b.Navigation("BudgetCategories");

                    b.Navigation("Periods");
                });

            modelBuilder.Entity("Finsight.Models.FSCategory", b =>
                {
                    b.Navigation("BudgetCategories");

                    b.Navigation("SubCategories");
                });

            modelBuilder.Entity("Finsight.Models.FSClosedTrade", b =>
                {
                    b.Navigation("InsurancePayout");
                });

            modelBuilder.Entity("Finsight.Models.FSTransaction", b =>
                {
                    b.Navigation("FSImportedTransactions");

                    b.Navigation("Files");
                });
#pragma warning restore 612, 618
        }
    }
}
